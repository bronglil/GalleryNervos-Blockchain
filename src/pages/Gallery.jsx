...
export default function Gallery(props) {
	...
	const GetTokenURIs = async (totalSupply) => {
		...
		for(let idx=totalSupply; idx>=1; idx--){
			try{
				...
				let response = await fetch(tokenURI.replace("ipfs://", "https://ipfs.io/ipfs/"));
	...
	return (
		<div className="page gallery">
			...
			<div style={{marginTop:'50px',display:'flex',flexDirection:'row',justifyContent:'center',gap:'15px',maxWidth:'100vw',flexWrap:'wrap'}}>
			{tokenURIs.map((uri, idx) => (
				<div key={idx}> 
					<img src={uri.image.replace("ipfs://", "https://ipfs.io/ipfs/")} alt={'token '+idx} style={{width:'220px',borderRadius:'10px',marginBottom:'0'}}/>
					<h3 style={{backgroundColor:'blue',borderBottomLeftRadius:'5px',borderBottomRightRadius:'5px',marginTop:'0'}}>{uri.name}</h3>
				</div>
